...
class initial_settings(Tool):
	...
	def create_action(self, for_toolbar=True):
		...
		# the plugins menu
		ac = QAction(get_icons('images/cogwheel.jpg'), 'تنظیمات اولیه تبدیل از ایندیزاین', self.gui)
		if not for_toolbar:
			# Register a keyboard shortcut for this toolbar action. We only
			# register it for the action created for the menu, not the toolbar,
			# to avoid a double trigger
			self.register_shortcut(ac, 'initial_changes', default_keys=('Ctrl+Shift+Alt+E',))
		ac.triggered.connect(self.initial_changes)
		return ac
		...
		for name, media_type in container.mime_map.iteritems():
			if media_type in OEB_DOCS:
				for item in container.parsed(name).getroottree().iter():
					...
					if "span" in str(item.tag) and item.text==" " and "nim_fasele" in str(item.get("class")):
